...
def main(args):
    ...
    # -- CRITERTION
    memax_weight = (
        1 if "memax_weight" not in args["criterion"] else args["criterion"]["memax_weight"]
    )
    ent_weight = 1 if "ent_weight" not in args["criterion"] else args["criterion"]["ent_weight"]
    freeze_proto = (
        False if "freeze_proto" not in args["criterion"] else args["criterion"]["freeze_proto"]
    )
    ...
    # -- init losses
    msn = init_msn_loss(
        num_views=focal_views + rand_views, tau=temperature, me_max=reg, return_preds=True
    )
    ...
    def one_hot(targets, num_classes, smoothing=label_smoothing):
        ...
        return torch.full((len(targets), num_classes), off_value, device=device).scatter_(
            1, targets, on_value
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr, (udata, _) in enumerate(unsupervised_loader):
            ...
            (loss, ploss, rloss, eloss, _logs, _new_lr, _new_wd, grad_stats), etime = gpu_timer(
                train_step
            )
...
def init_opt(
    ...
    wd_scheduler = CosineWDSchedule(
        optimizer, ref_wd=wd, final_wd=final_wd, T_max=int(1.25 * num_epochs * iterations_per_epoch)
    )
